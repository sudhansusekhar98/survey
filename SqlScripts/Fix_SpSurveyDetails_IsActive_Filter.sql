-- =============================================
-- Fix: Add IsActive filter to SpSurveyDetails SpType=4
-- This fixes two issues:
--   1. Inactive items showing in survey form
--   2. Ensures newly added active items appear correctly
-- =============================================
USE [VLDev]
GO
...
BEGIN TRY
    ...
    --    FIX: Added IsActive = '1' filter to exclude inactive items
    ------------------------------------------------------------
    IF @SpType = 4
    BEGIN
        SELECT
        ...
        (
            ...
            WHERE TypeId = @ItemTypeID
              AND IsActive = '1'    -- FIX: Only show active items
        ) A
        LEFT JOIN
        (
            SELECT
