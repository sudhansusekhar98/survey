...
{
    public class PowerReportChart : IDocument
    {
        ...
        void BuildHeader(IContainer container)
        {
            ...
            container.Column(col =>
            {
                ...
                col.Item().PaddingTop(10).Border(1).BorderColor(Colors.Grey.Lighten1).PaddingTop(5)
                .Row(row =>
                {
                    row.RelativeItem(3).AlignMiddle().PaddingLeft(5).PaddingRight(5).PaddingBottom(5).PaddingTop(0)
                        .Image(HeaderChartImage).FitWidth();
                    
                    // Vertical separator (1px wide)
                    row.ConstantItem(1).AlignMiddle().Height(100).Background(Colors.Grey.Lighten3);

                    row.RelativeItem(1).AlignMiddle().PaddingLeft(5).PaddingRight(5).PaddingBottom(5).PaddingTop(0)
                        .Image(AuxChartImage).FitWidth();
                });
            });
        }
            ...
            container.PaddingVertical(0).Column(col =>
            {
                col.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Element(CellHeaderStyle).Text("PUMP").AlignCenter();
