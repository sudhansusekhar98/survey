...
-- Database: VLDev
-- Purpose: Add SqNo to SpSurveyDetails SpType=4 query for item sorting
-- =============================================
-- Command: sqlcmd -S 10.0.32.135 -U adminrole -P @dminr0le -d VLDev -i SQL_Add_SqNo_ToItemList.sql
-- =============================================
...
BEGIN TRY
    ...
    ------------------------------------------------------------
    -- 4. Get All Items for ItemType + Existing SurveyDetails (with SqNo for sorting)
    ------------------------------------------------------------
    IF @SpType = 4
    BEGIN
        SELECT
            ...
            , ItemCode
            , ItemDesc
            , ISNULL(A.SqNo, 0) AS SqNo
            , ISNULL(ItemQtyExist, 0) AS ItemQtyExist
            , ISNULL(ItemQtyReq, 0) AS ItemQtyReq
            , ImgID
            , ImgPath
            , B.Remarks
        FROM
        ...
        ORDER BY 
            CASE WHEN A.SqNo > 0 THEN A.SqNo ELSE 999999 END,
            A.ItemId;
    END
    ...
    BEGIN
        SELECT 
            sls.*,
            u.EmpName AS CompletedByName,
            u2.EmpName AS VerifiedByName
        FROM SurveyLocationStatus sls
        LEFT JOIN EmpMaster u ON sls.CompletedBy = u.EmpID
        LEFT JOIN EmpMaster u2 ON sls.VerifiedBy = u2.EmpID
...
PRINT 'SpSurveyDetails updated successfully - SpType=4 now includes SqNo and ORDER BY'
GO
